...
조건 :
# 대화 시전시 데이터값 더미값 넣기 # unless @s 0.1.. + set 0
...
- 함수를 시전자마다 개별로 생성하면 함수가 많아질수록 새로운 시전자를 생성할수 없음   # 함수 생성시 복붙을 못하고 지정 범위가 증가함 ( 비쥬얼스튜디오로 Ctrl + Shift + H 쓰면 될듯?)
...
1. 함수단위로 복붙시 새로운 대상자 추가시 전부 일일히 복붙해야함
2. 네임스페이스 단위로 복붙시 새로운 명령어 추가시 파일 전부에 넣어줘야함 
